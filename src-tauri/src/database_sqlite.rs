...
async fn get_stats(
    ...
    if let Some(user) = authenticated_user.as_ref() {
        ...
        Ok(StatsResponse {
            ...
            uploads_today: stats.1,
            total_size: format!("{} bytes", stats.2),
            active_documents: stats.0,
        })
    } else {
...
async fn get_documents(
    ...
    if let Some(user) = authenticated_user.as_ref() {
        ...
        let document_responses: Vec<DocumentResponse> = documents.into_iter().map(|doc| {
            DocumentResponse {
                id: doc.id,
                name: doc.name,
                size: doc.file_size,
                file_type: doc.file_type,
                upload_date: doc.created_at.format("%d/%m/%Y %H:%M").to_string(),
                is_active: true,
...
pub fn run() {
    ...
    log::info!("🚀 Iniciando ARKIVE Desktop Application...");

    // Criar runtime do Tokio
